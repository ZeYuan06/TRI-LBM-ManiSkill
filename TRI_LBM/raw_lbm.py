...
from x_transformers import Encoder, TransformerWrapper, ContinuousTransformerWrapper
...
from bidirectional_cross_attention import (
    BidirectionalCrossAttentionTransformer as BiCrossAttnTransformer,
)
...
class ActionClassifier(Module):
    def __init__(
        ...
        *,
        ...
        dim=256,
        depth=3,
        attn_kwargs: dict = dict(attn_dim_head=64, heads=8),
        ...
        self.to_tokens = nn.Sequential(nn.Linear(dim_action, dim))

        self.transformer = Encoder(dim=dim, depth=depth, **attn_kwargs)

        self.to_logits = nn.Sequential(
            Reduce("b n d -> b d", "mean"), nn.Linear(dim, num_action_types)
        )
        ...
        if exists(action_variance):
            ...
            action_sum_diff_squared = einx.multiply(
                "b d, b", action_variance, action_counts - 1.0
            )

        self.register_buffer(
            "action_counts", default(action_counts, torch.zeros(num_action_types))
        )
        self.register_buffer(
            "action_mean",
            default(action_mean, torch.zeros(num_action_types, dim_action)),
        )
        self.register_buffer(
            "action_sum_diff_squared",
            default(action_sum_diff_squared, torch.zeros(num_action_types, dim_action)),
        )
    ...
    def action_variance(self):
        return einx.divide(
            "b d, b", self.action_sum_diff_squared, self.action_counts - 1
        )

    def get_stats_from_dataset_(self, dataset, batch_size=32, parallel=True):
        dataloader = DataLoader(dataset, batch_size=batch_size)

        update_stats_fn = (
            self.update_action_statistics_with_parallel_welford_
            if parallel
            else self.update_action_statistics_with_welford_
        )

        for actions, action_types in dataloader:
            update_stats_fn(actions, action_types)

    def standardize_shapes(self, actions, action_types):
        if actions.ndim == 3:
    ...
    def update_action_statistics_with_welford_(
        self, actions, action_types  # (b d) | (b t d)  # (b) | (b t)
    ):
    ...
    def update_action_statistics_with_parallel_welford_(
        ...
        old_sum_diff_squared, old_mean, old_count = (
            self.action_sum_diff_squared,
            self.action_mean,
            self.action_counts,
        )
        ...
        masked_repeated_actions = einx.where(
            "na b, na b d,", mask, repeated_actions, 0.0
        )
        ...
        diff_squared = einx.subtract(
            "na b d, na d", masked_repeated_actions, new_mean
        ).pow(2)

        masked_diff_squared = einx.where("na b, na b d,", mask, diff_squared, 0.0)

        new_sum_diff_squared = reduce(masked_diff_squared, "na b d -> na d", "sum")

        ratio = (count * old_count) / (next_count)

        next_sum_diff_squared = (
            old_sum_diff_squared
            + new_sum_diff_squared
            + einx.multiply("b, b ...", ratio, delta.pow(2))
        )
        ...
        self.action_sum_diff_squared.copy_(
            einx.where("b, b ...,", update_mask, next_sum_diff_squared, 0.0)
        )
        self.action_mean.copy_(einx.where("b, b ...,", update_mask, next_mean, 0.0))

    @torch.no_grad()
    def normalize(self, actions, action_types):
        ...
        inv_std = einx.where("b,, b d", counts == 0, 1.0, inv_std)
    ...
    @torch.no_grad()
    def inverse_normalize(self, normed_actions, action_types):
        counts = self.action_counts[action_types]
        means = self.action_mean[action_types]
        variances = self.action_variance[action_types]

        std = variances.clamp(min=1e-5).sqrt()
        std = einx.where("b,, b d", counts == 0, 1.0, std)

        normed_actions = einx.multiply("b t d, b d", normed_actions, std)
        actions = einx.add("b t d, b d", normed_actions, means)
        return actions

    def get_action_statistic(self, actions_dataset: Dataset, batch_size=16):
    ...
    def forward(
        ...
        ).any(), "you need to have run through the entire dataset for the action statistics before being able to train a classifier"
...
class DiffusionTransformerWrapper(Module):
    def __init__(self, dim_input, dim_time, transformer: Encoder):
        super().__init__()
...
class LBM(Module):
    def __init__(
        ...
        language_model, _, preprocess = open_clip.create_model_and_transforms(
        ...
        self.text_encodings_to_cross_attn_embed = (
            nn.Linear(dim_text_feats, additional_context_dim)
            if cross_attend_text_encodings
            else None
        )
        ...
        dim_observation = (
            dim_time + dim_text_feats + dim_image_feats * num_image_frames + dim_pose
        )
    ...
    def get_clip_text_image_feats(
        ...
        if exists(touch):
            assert exists(
                self.to_tactile_tokens
            ), f"`dim_tactile_input` must be set if tactile data is passed in"

            tactile_tokens = self.to_tactile_tokens(touch)
            image_embeds, tactile_tokens = self.tactile_fusion(
                image_embeds, tactile_tokens
            )
    ...
    def sample(
        ...
        if images.shape[1:] != self.images_shape:
            raise ValueError(
                f"Expected images of shape (b, {self.images_shape}), got {images.shape}"
            )

        text, images, maybe_text_encodings, maybe_text_mask = (
            self.get_clip_text_image_feats(text, images, touch=touch)
        )
        ...
        sampled_actions, noise = self.gaussian_diffusion_1d.sample(
        ...
        if self.normalize_with_action_classifier:
            ...
            needs_chunking = (
                exists(self.action_chunk_size) and action_len > self.action_chunk_size
            )
            ...
            pred_action_types, sampled_actions = self.action_chunk_normalizer(
                sampled_actions
            )
    ...
    def forward(
        ...
        task_status: (
            Tensor | None
        ) = None,  # must be Int['b'] of {-1, 0, 1} - `-1` for invalid action / language pair
        vlm_key_values: list[tuple[Tensor, Tensor]] | None = None,
    ):
        ...
        if self.training and exists(maybe_text_mask):
            dropout_text_encoding = (
                torch.rand(batch, device=device) < self.dropout_text_encodings_prob
            )

            maybe_text_mask = einx.where(
                "b, , b n", dropout_text_encoding, False, maybe_text_mask
            )
        ...
        assert xnor(
            exists(depth_embed), self.accept_depth_embed
        ), f"`dim_depth_embed` must be set if `depth_embed` were passed in (batch, seq, <dim_depth_embed>)"
        ...
        assert xnor(
            self.normalize_with_action_classifier, exists(action_types)
        ), f"`action_types` must be passed in during training if `action_chunk_normalizer` is being used by the LBM, converse must be true as well"
        ...
        if self.normalize_with_action_classifier:
            ...
            if needs_chunking:
                ...
                actions = rearrange(
                    actions, "b (c t) d -> b c t d", t=self.action_chunk_size
                )
                action_types = repeat(
                    action_types,
                    "b c -> b (c r)",
                    r=action_len // self.action_chunk_size,
                )

                actions, action_types = tuple(
                    rearrange(t, "b c ... -> (b c) ...")
